var controls = function(game, laserFire) {
    ...
    // shoot button 
    if ((game.shoot.justDown || game.pad1.justPressed(Phaser.Gamepad.XBOX360_B)  && !game.player1Sprite.holdFire && game.player1Sprite.weapon)    ) {
...
// pad2
    // dpad left
    if (game.pad2.isDown(Phaser.Gamepad.XBOX360_DPAD_LEFT) || game.pad2.axis(Phaser.Gamepad.XBOX360_STICK_LEFT_X) < -0.1) {

        game.player2Sprite.body.velocity.x = -walkspeed;
        game.player2Sprite.facing = 'left'
        
    // dpad right
    } else if (game.pad2 && game.pad2.isDown(Phaser.Gamepad.XBOX360_DPAD_RIGHT) || (game.pad2 && game.pad2.axis(Phaser.Gamepad.XBOX360_STICK_LEFT_X)) > 0.1) {
    ...
    // jump button
    if (game.pad2.justPressed(Phaser.Gamepad.XBOX360_A) && (game.player2Sprite.body.onFloor() || game.player2Sprite.body.touching.down) ) {
        game.player2Sprite.body.velocity.y = -jumpStrength;
    }

    // shoot button 
    if (game.pad2.justPressed(Phaser.Gamepad.XBOX360_B) && !game.player2Sprite.holdFire ) {
